...
const LoadMoreHotJobs = ({ previousJobs }: LoadMoreProps) => {
  ...
  async function loadMore() {
    setIsLoading(true);
    try {
      const newJobs = await getJobs({ is_hot: true, skip: previousJobs.length, limit: 25 });
  ...
  return (
    <Button onClick={loadMore} disabled={jobsEnded}>
      LoadMore
      <RefreshCcwIcon className={cn('ml-2 h-4 w-4', { 'animate-spin': isLoading })} />
    </Button>
  );
};
export default LoadMoreHotJobs;
