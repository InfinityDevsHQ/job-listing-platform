...
export default function JobCard({ job }: JobProps) {
  ...
  return (
    <div
      className="flex cursor-pointer flex-col gap-4 rounded-md border border-dark-background3 bg-dark-primary p-4 lg:p-8"
      onClick={() => setIsOpened(!isOpened)}
    >
      <JobCardHeader title={job.title || ''} is_hot={job.is_hot || false} company="Semrush" />
      <div className="grid grid-cols-3 items-center justify-between font-semibold text-dark-name lg:flex">
        <span className="flex items-center gap-2.5">
          <Clock className="h-2.5 w-3 lg:h-3 lg:w-3.5" />
          <span className="text-xxs capitalize lg:text-sm">{job.employment_type}</span>
        </span>

        <span className="hidden h-full w-0.5 bg-dark-name lg:inline-block"></span>
        <span className="flex items-center gap-2.5">
      ...
      <AnimatePresence>
        {isOpened && (
          ...
          >
            <p className="h-[116px] max-w-full overflow-hidden whitespace-pre-wrap break-words text-sm text-dark-name lg:h-auto">
              {job.description}
            </p>
            <span className="border border-dark-name"></span>
