...
export const AddJobListingDataSchema = z
	.object({
		title: z.string().min(2, "Title should have at least 2 characters.").max(99, "Title should not exceed 99 characters."),
		city: z.string().nonempty("City cannot be empty.").max(50, "City should not exceed 50 characters."),
		country: z.string().refine((value) => value !== "-", { message: "Must be selected." }),
		language: z.string(),
		employment_type: z.enum(["Full-time", "Part-time", "Contract", "Volunteer", "Seasonal", "Freelance"]),
		...
		description: z.string().nonempty("Description cannot be empty.").max(4096, "Description should not exceed 4096 characters."),
	})
	.refine(
		(data) => {
...
export const GeneratedJobDetailDataSchema = z
	.object({
		title: z.string().min(2, "Title should have at least 2 characters.").max(130, "Title should not exceed 130 characters."),
		city: z.string().nonempty("City cannot be empty.").max(50, "City should not exceed 50 characters."),
		country: z.string().refine((value) => value !== "-", { message: "Must be selected." }),
		language: z.string(),
		employment_type: z.enum(["Full-time", "Part-time", "Contract", "Volunteer", "Seasonal", "Freelance"]),
...
export const ContactsSchema = z.object({
	phone: z.string(),
	linkedIn: z.union([z.string().url({ message: "URL must include https://" }), z.literal(""), z.undefined()]),
	twitter: z.union([z.string().url({ message: "URL must include https://" }), z.literal(""), z.undefined()]),
	github: z.union([z.string().url({ message: "URL must include https://" }), z.literal(""), z.undefined()]),
});
...
export const EditProfileCardDataSchema = z.object({
	...
	linkedIn: z.union([urlSchema, z.literal(""), z.undefined()]),
	twitter: z.union([urlSchema, z.literal(""), z.undefined()]),
	github: z.union([urlSchema, z.literal(""), z.undefined()]),
	resume: z.any(),
});
